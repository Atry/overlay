...
\title{MIXIN: A Total Language with Lazy Evaluation and Automatic Termination}
\author{}
\date{}
...
\begin{abstract}
We present MIXIN, a programming language that achieves a unique combination of properties: compile-time Turing completeness, runtime totality (weak totality/productivity), support for circular references, and lazy evaluation. Unlike existing total languages that require manual termination proofs, MIXIN uses a simple naming convention combined with compile-time checks to automatically guarantee productivity. This makes MIXIN the first total language accessible to programmers without formal verification expertise.
\end{abstract}
...
\textbf{MIXIN guarantees weak totality:}
\begin{itemize}
...
\subsection{MIXIN's Innovation}

MIXIN achieves totality \textbf{automatically} through a simple naming convention:
\begin{itemize}
...
This naming convention, combined with MIXIN's tree-based symbol structure, guarantees totality without any manual proofs.

\section{Key Concepts}

\subsection{Symbol Tree}

Every MIXIN program's symbols form a \textbf{tree structure}:
...
\begin{itemize}
  ...
  \item MIXIN prohibits referencing the root from within nested scopes
...
In MIXIN, lazy evaluation ensures that accessing any finite depth terminates, even when the conceptual structure is infinite.

\subsection{Instantiation Levels}

To ensure weak totality, MIXIN uses a concept similar to \textbf{universe levels} in type theory.
...
MIXIN uses lazy evaluation instead of explicit guardedness, achieving the same totality guarantee with simpler syntax.

\section{MIXIN's Four Properties}
...
\begin{itemize}
  ...
  \item MIXIN uses \textbf{lazy evaluation + automatic guards}
\end{itemize}
...
\textbf{MIXIN verifies this automatically} by checking that:
\begin{itemize}
...
\hline
\textbf{MIXIN} & \checkmark & \checkmark & \checkmark & \checkmark \\
\hline
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Aspect} & \textbf{Idris/Agda/Coq} & \textbf{MIXIN} \\
\hline
...
\section{Why MIXIN's Combination is Unique}
...
\begin{itemize}
  ...
  \item \textbf{MIXIN}: \textbf{Total + Lazy + Automatic}
\end{itemize}

\subsection{The Secret: Tree Structure + Naming Convention + Level System}

MIXIN achieves automatic totality through:
...
MIXIN achieves a unique combination of properties:
\begin{enumerate}
...
MIXIN is the \textbf{first total language accessible to all programmers}:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Practical applications}: Verified software development using MIXIN's automatic totality
\end{itemize}

\section*{Acknowledgments}

This work builds on decades of research in dependent types, lazy evaluation, and termination analysis. We acknowledge the foundational work in Haskell, Agda, Coq, and Idris that inspired MIXIN's design.
