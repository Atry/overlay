...
\begin{itemize}
  ...
  \item $\references(p)$: the multiset of resolved reference pairs
    $(n,\; \ell_{\mathrm{down},*})$,
    where $n$ is a non-negative integer (the de~Bruijn index)
    and $\ell_{\mathrm{down},*}$ is a label list (downward projections).
\end{itemize}
...
\[
  ...
  \begin{cases}
    ...
    \begin{aligned}
      &p_{\mathrm{branch}} \in \supers(\init(p)), \\
...
\paragraph{Edges.}
The \emph{edges} of $p$ are the paths reachable by navigating
its resolved references one step:
\[
  \edges(p) =
...
Navigation proceeds in two phases: first, $n$ upward steps
resolve the definition-site path to composition-site paths;
then the downward projections $\ell_{\mathrm{down},*}$ are appended.
The auxiliary function $\navigateup$ performs the upward phase,
using $\qualifiedthis$ (defined below) to resolve each
definition-site path to its composition-site counterparts:
\[
  \navigateup(S,\; p_{\mathrm{def}},\; n) =
  \begin{cases}
    S & \text{if } n = 0 \\[6pt]
    \navigateup\!\left(
    \left\{\; p_{\mathrm{resolved}} \;\left|\;
    \begin{aligned}
      &p_{\mathrm{current}} \in S, \\
      &p_{\mathrm{resolved}} \in
...
The full navigation function applies $\navigateup$, then appends the
...
The recursion calls $\qualifiedthis$, which we now define.
