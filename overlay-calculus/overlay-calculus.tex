...
(Appendix~\ref{app:function-color-blind})---we found that the
...
(Appendix~\ref{app:function-color-blind}).
...
$\mathrm{S}(n) + m$ to $\mathrm{S}(n_0 + (m+1))$, where $n_0$
is the predecessor of $n$, by delegating to $n_0$'s $\mathrm{Plus}$:
\begin{align*}
...
The function color blindness property (Appendix~\ref{app:function-color-blind}) shows how Overlay-Calculus naturally
...
a static error (Appendix~\ref{app:scala-multi-path}).
...
\section{Function Color Blindness}
\label{app:function-color-blind}

% Previously titled "CPS-Agnostic Programs". Overlay-Calculus pseudocode preserved
% below (commented out) for future porting to .oyaml.

We implemented a web application as overlay files\selfcite{~\cite{mixin2025}}{~(supplementary material)}.
The same overlay file runs under both Python \texttt{asyncio} and
synchronous execution without modification---an instance of function color
blindness~\cite{nystrom2015color}, equivalently CPS-agnostic behavior.
We did not design for this.

In Bob Nystrom's framing~\cite{nystrom2015color}, a language suffers from
``function color'' when async functions and sync functions are distinct
types and cannot be used interchangeably.
In our web application, the same overlay file works in both execution
contexts because empty property slots are structurally identical to filled
ones: a slot left empty is an abstract dependency that an external
orchestrator can fill at assembly time.
Under \texttt{asyncio}, the orchestrator fills slots with coroutine
handlers; under synchronous execution, it fills the same slots with
direct implementations.
The overlay file itself sees no difference.

% --- Overlay-Calculus pseudocode (commented out; port to .oyaml before restoring) ---
%
...
% entire language.
% --- End of commented-out pseudocode ---
