...
\title{the Overlay language: A Total Language with Lazy Evaluation and Automatic Termination}
\author{}
\date{}
...
\begin{abstract}
We present the Overlay language, a programming language that achieves a unique combination of properties: compile-time Turing completeness, runtime totality (weak totality/productivity), support for circular references, and lazy evaluation. Unlike existing total languages that require manual termination proofs, the Overlay language uses a simple naming convention combined with compile-time checks to automatically guarantee productivity. This makes the Overlay language the first total language accessible to programmers without formal verification expertise.
\end{abstract}
...
\textbf{the Overlay language guarantees weak totality:}
\begin{itemize}
...
\subsection{the Overlay language's Innovation}

The Overlay language achieves totality \textbf{automatically} through a simple naming convention:
\begin{itemize}
...
This naming convention, combined with the Overlay language's tree-based symbol structure, guarantees totality without any manual proofs.

\section{Key Concepts}

\subsection{Symbol Tree}

Every the Overlay language program's symbols form a \textbf{tree structure}:
...
In the Overlay language:
\begin{itemize}
  \item The entire symbol tree, constructed at compile time, defines the static structure
  \item the Overlay language prohibits referencing the root from within nested scopes
...
In the Overlay language, lazy evaluation ensures that accessing any finite depth terminates, even when the conceptual structure is infinite.

\subsection{Instantiation Levels}

To ensure weak totality, the Overlay language uses a concept similar to \textbf{universe levels} in type theory.
...
The Overlay language uses lazy evaluation instead of explicit guardedness, achieving the same totality guarantee with simpler syntax.

\section{the Overlay language's Four Properties}
...
\begin{itemize}
  ...
  \item the Overlay language uses \textbf{lazy evaluation + automatic guards}
\end{itemize}
...
\textbf{the Overlay language encoding:}
\begin{lstlisting}[style=yaml]
Nat:
  Zero:
    Addition:
...
\textbf{the Overlay language verifies this automatically} by checking that:
\begin{itemize}
...
\hline
\textbf{the Overlay language} & \checkmark & \checkmark & \checkmark & \checkmark \\
\hline
...
\textbf{the Overlay language (automatic):}
\begin{lstlisting}[style=yaml]
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Aspect} & \textbf{Idris/Agda/Coq} & \textbf{the Overlay language} \\
\hline
...
\section{Why the Overlay language's Combination is Unique}
...
\begin{itemize}
  ...
  \item \textbf{the Overlay language}: \textbf{Total + Lazy + Automatic}
\end{itemize}

\subsection{The Secret: Tree Structure + Naming Convention + Level System}

The Overlay language achieves automatic totality through:
...
The Overlay language achieves a unique combination of properties:
\begin{enumerate}
...
The Overlay language is the \textbf{first total language accessible to all programmers}:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Practical applications}: Verified software development using the Overlay language's automatic totality
\end{itemize}

\section*{Acknowledgments}

This work builds on decades of research in dependent types, lazy evaluation, and termination analysis. We acknowledge the foundational work in Haskell, Agda, Coq, and Idris that inspired the Overlay language's design.
