...
every execution strategy (eager or lazy, with or without
memoization) yields the same result, so the gap between
semantics and execution is trivial.
...
when the inductive fixed point already captures divergence as
the absence of finite derivations, no separate coinductive
