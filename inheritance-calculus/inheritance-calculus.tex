...
A mixin tree has a single abstraction, the mixin:
every mixin can serve as field, method, class, and object
at the same time in object-oriented terms.

Unlike the $\lambda$-calculus, which requires $\beta$-reduction
to specify how terms compute, inheritance-calculus has
\emph{no reduction steps}: no rewriting of terms, no
...
to nonextensibility.
We now ask whether an alternative semantics could share this
immunity.
Each candidate we examine turns out to be vulnerable
to nonextensibility in a specific way.
...
\begin{description}
  \item[Functions as unit of reuse]
    ...
    This opacity is the root vulnerability:
    independently authored closures with the same label cannot
    be merged, so extensibility must be encoded via separate
    machinery.
    ...
    Once functions are excluded as the unit of reuse,
    the remaining four equations are determined by the syntax:
    ...
    closure, and $\overrides$~(\ref{eq:overrides}) merges
    same-label definitions---the deep merge that opacity
    prevented.
    The only remaining degree of freedom is how references
    resolve: the $\resolve$~(\ref{eq:resolve}) and
    $\this$~(\ref{eq:this}) equations.
    The next three candidates vary this dimension.
  ...
  \item[Dynamic scope]
    ...
    specifically, the Substitution Lemma
    (Lemma~\ref{lem:substitution}) that underpins full
    abstraction (Appendix~\ref{app:expressiveness}).
    The lemma's base case relies on
    $\resolve$~(\ref{eq:resolve}) starting from
    the definition-site path $\init(p_{\mathrm{def}})$ and
    $\this$~(\ref{eq:this}) navigating up exactly~$n$ scope
    levels (the de~Bruijn index determined at the definition site)
    to reach the binding $\lambda$.
    With dynamic scope, both would be replaced by a runtime
    context, and a reference to variable~$y$ ($y \neq x$)
    could resolve to the $\mathrm{argument}$ slot
    intended for~$x$, violating the variable-separation
    property on which the induction depends.
...
\noindent
None of the candidates above is immune to nonextensibility.
The first candidate prevents same-level merging;
the next two prevent the mixin from replacing functions;
the last prevents cross-level (nested) extensions from coexisting.
The mechanisms they require---MRO,
framework encodings, runtime context threading,
duplicate-declaration errors---are absent from the equations of
Section~\ref{sec:mixin-trees}.
We are not aware of a semantic alternative that warrants
...
Immunity to nonextensibility forced the semantics
