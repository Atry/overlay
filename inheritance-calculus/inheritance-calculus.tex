...
\begin{itemize}
  ...
  \item Open extension via inheritance changes the operational
    ...
    that inheritance-calculus is strictly more expressive than
    its sublanguage isomorphic to the lazy $\lambda$-calculus
    (Section~\ref{sec:asymmetry}).
\end{itemize}
...
Inheritance-calculus has a single abstraction, the mixin,
...
This section defines the semantics as a
\emph{fixpoint semantics}~\cite{vanemden1976semantics,aczel1977inductive}:
six mutually recursive set-comprehension equations over
paths whose least fixed point on a product of powerset
lattices is guaranteed by the Knaster--Tarski
...
\subsection{Observational Fixpoint Semantics}
Recall that fixpoint semantics defines meaning via recursive
equations on a lattice: one writes a system of equations,
verifies that the induced operator is monotone,
and takes the least fixed point guaranteed by the
Knaster--Tarski theorem~\cite{tarski1955lattice}.
Meaning is given directly by the fixed point, without
reduction rules or evaluation contexts.
The technique originates in logic
programming~\cite{vanemden1976semantics} and the theory
of inductive definitions~\cite{aczel1977inductive}.

The six equations
(\ref{eq:properties})--(\ref{eq:this}) are such a system:
they define set membership inductively on a product of
powerset lattices indexed by paths.
$\ell \in \properties(p)$ holds if and only if it can
be established by a finite chain of applications of
these equations.
The corresponding operator is monotone; the detailed
well-definedness argument, which identifies the lattice,
verifies positivity of each equation, and handles the
transitive closure $\bases^*$, is given in
Appendix~\ref{app:well-definedness}.
When no finite derivation exists for a given query, the
membership is absent from the least fixed point and
the semantics is undefined at that point, analogous to
divergence in the $\lambda$-calculus.

This completes the semantics of inheritance-calculus:
the observer drives the computation by choosing which
path to inspect, and the equations unfold on demand.
The same equations double as computation rules
for an interpreter.%
\footnote{To implement such an interpreter, note that
  the equations are pure and order-independent: the
  reflexive-transitive closure $\bases^*$
  in~(\ref{eq:supers}) may be explored breadth-first or
  depth-first without affecting the semantics;
...
That fixpoint semantics on powerset lattices suffices for
full abstraction---without continuous lattices, inverse limits,
or game-theoretic machinery---is the central semantic result
of this paper.%
...
fresh labels (such as $\mathrm{result}$ and $\_\mathrm{tail}$
...
\begin{center}
  \begin{tabular}{lcccll}
    ...
    Conditional
    & \multicolumn{3}{c}{breaks monotonicity}
    & --- & --- \\
  \end{tabular}
\end{center}
...
\begin{description}
  ...
  \item[$\beta$-reduction]
    ...
    The frameworks that simulate universal extensibility
    (visitor pattern,
  ...
  \item[Conditional incorporation]
    Removing or conditionally including definitions breaks
    monotonicity, so the semantic equations
    (Appendix~\ref{app:well-definedness}) may have no
    fixed point~\cite{tarski1955lattice}.
\end{description}
...
nonextensibility or break monotonicity.
...
\paragraph{Fixpoint semantics on powerset lattices}
...
The $T_P$ operator is monotone on a complete lattice, exactly as
our six equations are.
Our setting differs in that the underlying lattice is a product
of powerset lattices indexed by \emph{paths}, and in that the
equations are mutually recursive rather than defined by a single
operator; but the mathematical machinery is the same.
...
providing the logical foundations that both
Datalog-style and our semantics rest on.
...
While abstract interpretation approximates the behavior of
programs, our equations \emph{define} the behavior exactly;
...
In our setting the analogous distinction is already captured by
the Knaster--Tarski theorem: the least fixed point contains
exactly the finitely derivable memberships (convergence), while
undecidable memberships (divergence) are absent from it---no
separate coinductive formulation is needed.

Each of these prior frameworks operates on its own domain
(Herbrand models, abstract domains, evaluation judgments).
To our knowledge, the present work is the first to show that the
same powerset-lattice fixed-point technique, applied to a
tree-structured domain of paths, yields a fully abstract semantics
of the lazy $\lambda$-calculus without function spaces,
substitution, or reduction rules, and simultaneously generalizes
to multi-path inheritance.
...
index~$n$.
Since $n$ decreases by one at each step and is a nonnegative
...
\section{Fixpoint Semantics of the ANF Lazy
\texorpdfstring{$\lambda$}{λ}-Calculus}
\label{app:lambda-fixpoint}

This appendix presents a fixpoint semantics of the lazy
$\lambda$-calculus in A-normal form (ANF) that lives entirely
in naive set theory.
The semantic domain is a product of powerset lattices;
the semantics is the least fixed point of a monotone operator,
...
\begin{align*}
\children(p) &= \{\_\mathrm{tail},\; \mathrm{result}\},
&\refs(p) &= \varnothing.
\end{align*}
The subtree at $p \snoc \_\mathrm{tail}$ is analogous to
...
&= \{(0,\; (\_\mathrm{tail},\; \mathrm{result}))\}.
\end{align*}
Here $(0,\; (\_\mathrm{tail},\; \mathrm{result}))$ navigates to
...
\paragraph{Observational semantics}
...
define set membership inductively:
$\ell \in \labels(p)$ holds if and only if it can be
established by a finite chain of applications of these
equations.
The corresponding operator on the product lattice of
powerset domains is monotone (every mutually defined function
appears only in positive position---as the domain of an
existential quantifier in a set comprehension---and no
complement, set difference, or negated membership test
appears).
Its least fixed point exists by the Knaster--Tarski
theorem~\cite{tarski1955lattice}.
When no finite derivation exists for a given query, the
membership is not in the least fixed point, analogous to
divergence.
