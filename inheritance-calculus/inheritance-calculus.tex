...
\footnote{In the $\lambda$-calculus, de~Bruijn
  ...
  term---that is, $k$ is the number of
  $\mathbf{let}$- and tail-call constructs on the path
  from the variable occurrence to its binding $\lambda$
  in the ANF syntax tree---its inheritance-calculus
  index is $n' = n + k$,
  not~$n$.
...
Second, the full $\mathrm{lfp}(T_P)$ semantics---the
fixpoint $\lambda$-calculus---is strictly more defined:
